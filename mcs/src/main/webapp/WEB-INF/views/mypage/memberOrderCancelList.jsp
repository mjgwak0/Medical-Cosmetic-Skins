...
<%--                                     <section class="ff"> --%>
										...
										<c:if test="${orderDetailList.size() > 0}">
                                  			<c:forEach var="orderDetail" items="${orderDetailList}">
                                  				<c:if test="${orderDetail.state eq '주문취소' || orderDetail.state eq '환불신청' || orderDetail.state eq '환불완료'}">
													<c:set var="a" value="1" />
                                        			<tbody>
                                        				<tr align="center">
                                            				<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${orderDetail.statedate}"/></td>
				                                            <td>${orderDetail.detailnum}</td>
                				                            <td>${orderDetail.product.pro_name}</td>
                                				            <td>${orderDetail.order_count}개</td>
				                                            <td><fmt:formatNumber value="${orderDetail.price}" pattern="#,###"/>원</td>
										...
										<c:if test="${a == 0 }">
                                    			<tbody>
                                    				<tr align="center">
                                    					<td colspan="6" style="padding: 50px 00px 50px 0px">취소/환불 내역이 없습니다.</td>
                                    				</tr>
                                    			</tbody>
                                    	</c:if>
                                    		</table>
